...
```ts
import { platformParams } from "@rpallas/platform-cdk-runtime";
const busName = await platformParams().env("dev").eventBusName();
```

## Adding a key

1. Add the path builder to `PlatformParameterPaths` in `packages/platform-cdk/src/params/paths.ts`.
2. Add the writer in the relevant `platform-cdk-account` module.
