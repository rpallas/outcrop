...
export const HELP = `outcrop - scaffolding and CI helpers for outcrop services

Usage: outcrop <command> [options]

Commands:
  create service <name>   Scaffold a new service (see: outcrop create service --help)
...
export const main = async (argv: string[], io: Io = processIo): Promise<number> => {
  ...
  if (first === "create") {
    ...
    if (!sub || sub === "--help" || sub === "-h") {
      io.out("Usage: outcrop create <service|account> <name> [options]");
      return sub ? 0 : 1;
    }
    key = `create:${sub}`;
    args = subRest;
