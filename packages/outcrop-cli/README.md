...
Scaffolding and CI helpers for services built with [`@rpallas/outcrop`](../outcrop).

```sh
npx @rpallas/outcrop-cli create service orders --http-api --dynamodb --auth jwt-auth0
npx @rpallas/outcrop-cli create account my-platform --owner my-org --domain example.com
npx @rpallas/outcrop-cli preview-id --branch feature/ABC-123-thing --pr 42   # abc-123
npx @rpallas/outcrop-cli stack-outputs --outputs-file cdk-outputs.json --prefix PREVIEW_ --github-env
npx @rpallas/outcrop-cli sync-skills
```
...
| `create account <name>` | Generates an account baseline app using `@rpallas/outcrop-account`.                            |
...
(`rpallas/outcrop/.github/workflows/*.yml@v1`) and expect these repository variables:
`AWS_REGION`, `AWS_DEPLOY_ROLE_ARN_DEV` (and `_PROD`, `AWS_READONLY_ROLE_ARN_DEV` for diffs).

Run `outcrop create service --help` for the full option list.
